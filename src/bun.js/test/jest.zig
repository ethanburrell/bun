...
pub const Expect = struct {
    ...
    pub fn toMatchSnapshot(this: *Expect, globalObject: *JSC.JSGlobalObject, callframe: *JSC.CallFrame) callconv(.C) JSValue {
        ...
        var snapshot_file = this.scope.snapshot;
...
pub const DescribeScope = struct {
    ...
    pub fn describe(
        ...
        var file_fs: Fs.Path = Jest.runner.?.files.items(.source)[this.file_id].path;
        const snapshot_path = snapshot.resolveSnapshotPath(file_fs, getAllocator(ctx));
        var snapshot_file: *snapshot.SnapshotFile = getAllocator(ctx).create(snapshot.SnapshotFile) catch unreachable;

        snapshot_file.* = .{ .path = snapshot_path, .globalObject = null, .allocator = getAllocator(ctx), .snapshotData = JSC.JSValue.jsUndefined(), .counters = snapshot.SnapshotFile.CountersMap.init(getAllocator(ctx)) };
    ...
    pub fn runTests(this: *DescribeScope, this_object: JSC.JSValue, ctx: js.JSContextRef) void {
        ...
        // If we are at the parent describe for the file save snapshot
        if (this.parent) |_| {
            std.debug.print("describe for test", .{});
        } else {
            std.debug.print("Closing describe for test", .{});
        }
    }
